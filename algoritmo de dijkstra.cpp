...
//implemetação do algoritmo de Dijkstra
void dijkstra(int vertices, int origem, int destino, int *custos){
	int i, v, cont = 0;
	int *ant, *tmp;
	int *z; // vertices para os quais se conhece o caminho minino
	double min;
...
void grafo_criar(void){
	...
	//Se o compilador falhou em alocar espaço de memória
	if (custos ==NULL){
		system("color fc");
...
//Buca os menores caminhos entre os vertices
void grafo_procurar(void){
	int i, j;
	system("cls");
	system("color 03");
